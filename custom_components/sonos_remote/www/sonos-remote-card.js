class SonosRemoteCard extends HTMLElement {
  setConfig(config) {
    ...
    this._defaultZoneOpen = this._defaultZoneOpen || false;
  ...
  _defaultZoneSettingsHtml() {
    if(!this._defaultZoneOpen)return "";
    const selected=this._backendInfo?.default_zone||"";
    const zones=this._nuvoZones();
    return `<div class="fixedsettings"><div class="fixedtitle"><b>Default Nuvo zone</b><button id="closedefaultzone"><ha-icon icon="mdi:close"></ha-icon></button></div><small>Choose which Nuvo zone opens by default on the Now Playing screen.</small>${zones.map(z=>`<label class="fixedrow"><span>${this._esc(z.attributes?.friendly_name||z.entity_id)}</span><input type="radio" name="default-nuvo-zone" data-default-zone="${z.entity_id}" ${selected===z.entity_id?"checked":""}></label>`).join("")}<label class="fixedrow"><span>Use first available zone</span><input type="radio" name="default-nuvo-zone" data-default-zone="" ${!selected?"checked":""}></label></div>`;
  }
  ...
  _hybridChoice() {
    ...
    const defaultZone=this._backendInfo?.default_zone;
    return choices.find(x=>x.id===this._hybridTarget)||choices.find(x=>x.kind==="nuvo"&&x.entities?.includes(defaultZone))||choices.find(x=>x.kind==="nuvo")||choices[0]||null;
  }
  _hybridContext() {
  ...
  _render() {
    ...
    this.shadowRoot.querySelector("#defaultzoneopen")?.addEventListener("click",e=>{e.stopPropagation();this._nowMenuOpen=false;this._defaultZoneOpen=true;this._render();});
    this.shadowRoot.querySelector("#closedefaultzone")?.addEventListener("click",e=>{e.stopPropagation();this._defaultZoneOpen=false;this._render();});
    this.shadowRoot.querySelectorAll("[data-default-zone]").forEach(el=>el.addEventListener("change",async e=>{
      e.stopPropagation();
      const entityId=e.target.dataset.defaultZone||null;
      try{
        const r=await this._hass.callWS({type:"sonos_remote/set_default_zone",entity_id:entityId});
        this._backendInfo={...(this._backendInfo||{}),default_zone:r.default_zone||null};
        if(entityId) this._hybridTarget=`nuvo:${entityId}`;
        else this._hybridTarget=null;
        this._defaultZoneOpen=false;
        this._render();
      }catch(err){
        this.dispatchEvent(new CustomEvent("hass-notification",{detail:{message:err?.message||"Unable to save default Nuvo zone"},bubbles:true,composed:true}));
      }
    }));
