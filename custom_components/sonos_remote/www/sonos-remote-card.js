class SonosRemoteCard extends HTMLElement {
  ...
  _defaultPlayerSettingsHtml() {
    ...
    return `<div class="fixedsettings"><div class="fixedtitle"><b>Default Sonos player</b><button id="closedefaultplayer"><ha-icon icon="mdi:close"></ha-icon></button></div><small>Choose which Sonos player opens by default and streams the music for the remote.</small>${(this._players||[]).map(p=>`<label class="fixedrow"><span>${this._esc(p.attributes?.friendly_name||p.entity_id)}</span><input type="radio" name="default-sonos-player" data-default-player="${p.entity_id}" ${selected===p.entity_id?"checked":""}></label>`).join("")}<label class="fixedrow"><span>Use first available Sonos player</span><input type="radio" name="default-sonos-player" data-default-player="" ${!selected?"checked":""}></label></div>`;
  }
  ...
  _hybridChoice() {
    ...
    const defaultPlayer=this._backendInfo?.default_player;
    return choices.find(x=>x.id===this._hybridTarget)||choices.find(x=>x.kind==="sonos"&&x.id===defaultPlayer)||choices.find(x=>x.kind==="nuvo")||choices[0]||null;
  }
  _hybridContext() {
  ...
  _render() {
    ...
    this.shadowRoot.querySelectorAll("[data-default-player]").forEach(el=>el.addEventListener("change",async e=>{
      ...
      try{
        ...
        if(entityId&&this._hass.states[entityId]){this._selected=entityId;this._hybridTarget=entityId;}
