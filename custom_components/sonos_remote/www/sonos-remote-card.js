class SonosRemoteCard extends HTMLElement {
  ...
  _nuvoZonesHtml() {
    ...
    return `<div class="nuvohead"><span>Nuvo Zones</span><small>${zones.length} zone${zones.length===1?"":"s"}</small></div><div class="nuvolist">${zones.map(z=>{const st=this._hass.states[z.entity_id],a=st?.attributes||z,v=Math.round((a.volume_level??z.volume_level??0)*100),off=(st?.state||z.state)==="off",src=a.source||z.source||"",sources=a.source_list||z.source_list||[];return `<div class="nuvozone"><div class="nuvotop"><button class="nuvopower ${off?"":"on"}" data-nuvo-power="${z.entity_id}"><ha-icon icon="mdi:power"></ha-icon></button><div class="nuvoname"><b>${this._esc(a.friendly_name||z.name||z.entity_id)}</b><small>${off?"Off":this._esc(src||"On")}</small></div></div><div class="nuvoctl"><ha-icon icon="mdi:volume-medium"></ha-icon><input data-nuvo-vol="${z.entity_id}" type="range" min="0" max="100" value="${v}" ${off?"disabled":""}><span>${v}</span></div>${sources.length?`<select data-nuvo-source="${z.entity_id}" ${off?"disabled":""}>${sources.map(x=>`<option value="${this._esc(x)}" ${x===src?"selected":""}>${this._esc(x)}</option>`).join("")}</select>`:""}</div>`}).join("")}</div>`;
  }
...
console.info("%c SONOS REMOTE WITH NUVO %c v0.6.0 ","color:white;background:#03a9f4;font-weight:bold","color:#03a9f4;background:white");
